...
$result = $conn->query("SELECT firstname, lastname, gender, patient_address, patient_number, birthdate, department, position, civil_status, blood_type FROM patient_pd_tbl WHERE id=$id");
	...
	$row = mysqli_fetch_assoc($result);
 	...
 	<div class="row">
		...
		<?php 
		$diagnosis = $conn->query("SELECT diagnosis, date_checkup FROM consultation_tbl WHERE patient_id=$id");

		$row = mysqli_fetch_array($diagnosis);
		...
		<div class="col-md-9">
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="row">
							<div class="col-md-12">
								<p><small style="color: red;"><i>*This diagnosis history was recorded on <?php echo $row['date_checkup']; ?></i></small></p>					
							</div>
					</div>
				</div>
			</div>
